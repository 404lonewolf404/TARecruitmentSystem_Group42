...
<body>
    ...
    <div class="container">
            ...
            <% for (Position position : positions) { %>
                <div class="card">
                    ...
                    <div style="margin: 15px 0;">
                        ...
                        <p><%= position.getDescription() != null ? position.getDescription() : "�? %></p>
                    </div>
                    
                    <div style="margin: 15px 0;">
                        <p><strong>要求�?/strong></p>
                        <p><%= position.getRequirements() != null && !position.getRequirements().isEmpty() ? position.getRequirements() : "无特殊要�? %></p>
                    </div>
                    
                    <div style="margin: 15px 0;">
                        <p><strong>工作时长�?/strong> <%= position.getHours() %> 小时/�?/p>
                        <p><strong>招聘名额�?/strong> <%= position.getMaxPositions() %> �?/p>
                        <p><strong>状态：</strong> 
                            <span class="badge badge-<%= position.getStatus().toString().toLowerCase() %>">
                                <%= position.getStatus() == com.bupt.tarecruitment.model.PositionStatus.OPEN ? "开�? : "关闭" %>
                            </span>
                        </p>
                    </div>
                    ...
                    <% } else { %>
                                ...
                                <div style="margin-bottom: 10px;">
                                    <label style="display: block; cursor: pointer;">
                                        ...
                                        <% if (currentUser.getCvPath() == null || currentUser.getCvPath().trim().isEmpty()) { %>
                                            <span style="color: #e74c3c; font-size: 0.9em;">(您还没有上传简�?</span>
                                        <% } %>
                                    </label>
                                </div>
                        ...
                        function validateResumeSelection(form) {
                            ...
                            if (resumeChoice === 'existing') {
                                <% if (currentUser.getCvPath() == null || currentUser.getCvPath().trim().isEmpty()) { %>
                                    alert('您还没有上传简历，请先在个人资料中上传简历或选择上传新简�?);
                                    return false;
                                <% } %>
        ...
        <div class="text-center mt-20">
            <a href="<%= request.getContextPath() %>/ta/dashboard" class="btn btn-secondary">返回仪表�?/a>
        </div>
    </div>
